...
partial class Program
{
    ...
    static void Query4(){
        ...
        using (Northwind db = new()){
            ...
            SectionTitle("Query4");
            var suplier = db.Suppliers;
            if ((suplier is null) || !suplier.Any())
            {
                Fail("No Categories found");
                return;
            }
            foreach (var Value in suplier)
            {
                WriteLine($"{Value.ContactName} , where contact title is {Value.ContactTitle}");
            }
        }
    }
